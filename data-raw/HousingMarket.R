...
SEAPopDataCleaning <- function(data,year)
{
  data%>%
    ...
    filter(`Label (Grouping)` %in% c("TOTAL POPULATION","18 YEARS AND OVER","HISPANIC OR LATINO (OF ANY RACE)", "WHITE ALONE", "BLACK OR AFRICAN AMERICAN ALONE","AMERICAN INDIAN AND ALASKA NATIVE ALONE", "NATIVE HAWAIIAN AND OTHER PACIFIC ISLANDER ALONE", "SOME OTHER RACE ALONE", "TWO OR MORE RACES"))%>%
    ...
    #add col for year
    mutate(Year = year,
           # Add a 'Location' column with the given location value
           Region = "Seattle")%>%
    select(20,19,1,2,11,3,12,4,13,5,14,6,15,7,16,8,17,9,18)
}
...
HousingMarket <- Housing %>%
  left_join(pop_data, by = c("Region", "Year"))
