...
AsianInteraction <-lm(Average.Sale.To.List ~`PopPct_ASIAN ALONE`*Region, data = HousingMarket)
#summary(lm4)
#plot(lm4)
anova(AsianInteraction)
overall_model <- lm(Average.Sale.To.List ~ `PopPct_ASIAN ALONE`, data = HousingMarket)

HousingMarket <- HousingMarket %>%
  dplyr::select( -matches('fit'), -matches('lwr'), -matches('upr') ) %>%
  cbind( predict(AsianInteraction, newdata=., interval='confidence') )%>%
  mutate(overall_fit = predict(overall_model, newdata = HousingMarket))


ggplot(HousingMarket , aes(x = `PopPct_ASIAN ALONE` , y = Average.Sale.To.List , color = Region)) +
