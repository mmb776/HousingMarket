...
WhiteInteraction <-lm(Average.Sale.To.List ~`PopPct_WHITE ALONE`*Region, data = HousingMarket)
#summary(WhiteInteraction)
#plot(lm4)
anova(WhiteInteraction)
overall_model <- lm(Average.Sale.To.List ~ `PopPct_WHITE ALONE`, data = HousingMarket)


HousingMarket <- HousingMarket %>%
  dplyr::select( -matches('fit'), -matches('lwr'), -matches('upr') ) %>%
  cbind( predict(WhiteInteraction, newdata=., interval='confidence') )%>%
  mutate(overall_fit = predict(overall_model, newdata = HousingMarket))

ggplot(HousingMarket , aes(x = `PopPct_WHITE ALONE` , y = Average.Sale.To.List , color = Region)) +
